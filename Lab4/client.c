...
int main(int argc, char *argv[]){
	...
	if(rv==0){ //timeout and no packet is received
		...
		PACKET snamePacket;
		recvfrom(sock, &snamePacket, sizeof(snamePacket), 0, NULL, NULL);

		if(snamePacket.header.seq_ack==!sequence){ 
	...
	while(!feof(fin)){
		...
		if(rv==0){ //timeout and no packet is received
			...
			PACKET serverPacket;
			recvfrom (sock, &serverPacket, sizeof(serverPacket), 0, NULL, NULL);
			if(serverPacket.header.seq_ack==!sequence){
	...
	*/
	//else if (rv==1){
		...
		PACKET rFinalPacket;
		recvfrom (sock, &rFinalPacket, sizeof(rFinalPacket), 0, NULL, NULL);
		if(rFinalPacket.header.seq_ack==!sequence){
		...
		printf("correct ACK has been received. \n");
		sequence=!sequence;
		count=0;
		//break; 
	//}
