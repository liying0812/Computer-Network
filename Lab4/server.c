...
int main(int argc, char *argv[]){
	...
	PACKET namePacket;
	while(recvfrom (sock, &namePacket, sizeof(namePacket), 0, (struct sockaddr *)&serverStorage, &addr_size)){
	...
	while (1){
		...
		//receive packet
		while (recvfrom(sock, &clientPacket, sizeof(clientPacket), 0, (struct sockaddr *)&serverStorage, &addr_size)){
		...
		if(clientPacket.header.length==0){
			...
			//if (rand()%10<=6){ //certain probability to send ACK
				//send packet back
			sendto(sock, &fPacket, sizeof(fPacket), 0, (struct sockaddr *)&serverStorage, addr_size);
		...
		if(temp==chksum){//checksum matched
			...
			if (rand()%10<=6){ //certain probability to send ACK
				...
				printf("ACK sent. \n");
				fwrite(clientPacket.data,1, clientPacket.header.length,fout);
				...
				if (rand()%10<=6){ //certain probability of losing ACK
